module JahanJooy.HaftDong.Dashboard {
    ...
    ngModule.controller("ProfileController", [
        ...
        ($scope, $rootScope, $http: ng.IHttpService, $state, $modal, messageBoxService, $stateParams,
            ...
            function initialize() {
                $http.get("/api/web/users/myprofile", null).success((data: any) => {
                    $scope.user = data;
            ...
            $scope.onAddNewEmail = () => {
                ...
                $http.post("/api/web/users/addnewcontactmethod", $scope.newContactMethodInputForApplicationUser).success((data: any) => {
                    $scope.user = data;
                    showPanels($scope.user);
                    fillContactMethodsList(data);
            ...
            $scope.onCompleteRegistrationVerifyEmailAddress = () => {
                var contactMethod = Enumerable.from($scope.user.Contact.Emails).first(e => !e.IsVerified);
                ...
                $http.post("/api/web/users/verifycontactmethod", $scope.UserApplicationVerifyContactMethodInput).success((data: any) => {
                    $scope.user = data;
                    showPanels($scope.user);
                    $scope.showAddEmailAddressPanel = false;
                    fillContactMethodsList(data);
                    ...
                    if (data.Errors.length > 0) {
                        var result = Enumerable.from(data.Errors).any(e => e.ErrorKey === "VerificationDeadlineExpired");
                        if (result) {
                            $scope.expiredEmailContactMethodVerificationDiv = true;
                            $scope.showAddEmailAddressPanel = false;
                            $scope.showEmailVerificationPanel = false;
                        }
                        result = Enumerable.from(data.Errors).any(e => e.ErrorKey === "InvalidSecret");
                        if (result) {
                            $scope.expiredEmailContactMethodVerificationDiv = false;
                            $scope.showAddEmailAddressPanel = false;
                            $scope.showEmailVerificationPanel = true;
            ...
            $scope.onCompleteRegistrationVerifyMobileNumber = () => {
                var contactMethod = Enumerable.from($scope.user.Contact.Phones).first(p => !p.IsVerified);
                $scope.UserApplicationVerifyContactMethodInput = {
                    UserID: $scope.user.Id,
                    ContactMethodID: contactMethod.ID,
            ...
            $scope.onRestartEmailAddressVerification = () => {
                var contactMethod = Enumerable.from($scope.user.ContactMethods).first(cm => cm.ContactMethodType === ContactMethodType.Email && !cm.IsVerified);
                $scope.NewSecretForUserApplicationInput = {
                    UserID: $scope.user.Id,
                    ContactMethodID: contactMethod.ID,
                    ContactMethodType: ContactMethodType.Email
                }
                $http.post("/api/web/users/getanothersecret", $scope.NewSecretForUserApplicationInput).success(() => {
                    $scope.showEmailVerificationPanel = true;
                    $scope.restartEmailAddressVerificationDiv = false;
                    $scope.expiredEmailContactMethodVerificationDiv = false;
                    ...
                    if (data.Errors.length > 0) {
                        var result = Enumerable.from(data.Errors).any(e => e.ErrorKey === "IsTooFrequentContactMethodVerification");
                        if (result) {
                            $scope.restartEmailAddressVerificationDiv = false;
                        }
                    }
            ...
            $scope.onDeleteMobileNumber = (contactId) => {
                $http.post("/api/web/users/deletecontactmethod/" + contactId,null).success((data: any) => {
                    $scope.user = data;
                    showPanels($scope.user);
                    fillContactMethodsList(data);
                    toastr.success("حذف تلفن همراه انجام شد.");
                }).error(() => {
                    toastr.error("حذف تلفن همراه با مشکل مواجه شد.");
                });
            }

            $scope.onDeleteEmailAddress = (contactId) => {
                $http.post("/api/web/users/deletecontactmethod/" + contactId, null).success((data: any) => {
                    $scope.user = data;
                    showPanels($scope.user);
                    fillContactMethodsList(data);
                    toastr.success("حذف ایمیل انجام شد.");
                }).error(() => {
                    toastr.error("حذف ایمیل  با مشکل مواجه شد.");
                });
            }
