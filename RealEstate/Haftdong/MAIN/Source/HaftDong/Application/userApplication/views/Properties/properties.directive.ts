module JahanJooy.HaftDong.Property {
    ...
    appModule.directive('jjProperty',
    () => {
        return {
            restrict: 'E',
            templateUrl: 'Application/userApplication/views/Properties/properties.directive.html',
            scope: {
                input: "=",
                onBackClick: '&',
                onSaveClick: '&'
            ...
            controller: ($scope,
                ...
                if ($scope.inEditMode) {
                    ...
                    $scope.customers = [$scope.input.Owner];
                } else {
                    $scope.vicinities = [];
                }
                    ...
                    p => {
                        if ($scope.input != null) {
                            if ($scope.input.IntentionOfOwner != null
                                && $scope.input.IntentionOfOwner === 6) {
                                ...
                                $scope.showEstatePanelProperty = Enumerable.from($scope.allowancesProperty)
                                    .any(a => a.propertyType === p &&
                                        Enumerable.from(a
                                            .panels)
                                        .any(at => at === $scope.allPropertyPanels.estatePanel));

                                $scope.showHousePanelProperty = Enumerable.from($scope.allowancesProperty)
                                    .any(a => a.propertyType === p &&
                                        Enumerable.from(a
                                            .panels)
                                        .any(at => at === $scope.allPropertyPanels.housePanel));

                                $scope.showUnitPanelProperty = Enumerable.from($scope.allowancesProperty)
                                    .any(a => a.propertyType === p &&
                                        Enumerable.from(a.panels).any(at => at === $scope.allPropertyPanels.unitPanel));

                                $scope.showShedPanelProperty = Enumerable.from($scope.allowancesProperty)
                                    .any(a => a.propertyType === p &&
                                        Enumerable.from(a.panels).any(at => at === $scope.allPropertyPanels.shedPanel));

                                $scope.showExtraHousePanelProperty = Enumerable.from($scope.allowancesProperty)
                                    .any(a => a.propertyType === p &&
                                        Enumerable.from(a
                                            .panels)
                                        .any(at => at === $scope.allPropertyPanels.extraHousePanel));

                                $scope.showShopPanelProperty = Enumerable.from($scope.allowancesProperty)
                                    .any(a => a.propertyType === p &&
                            ...
                            $scope.input.Rent = null;
                            $scope.input.Mortgage = null;
                ...
                $scope.$watch("input.UsageType",
                    u => {
                            if ($scope.input != null) {
                                ...
                                if (u != null) {
                                    usagePropertyTypes = Enumerable.from(allPropertiesPerUsage)
                                ...
                                if ($scope.input.IntentionOfOwner != null) {
                                    intentionPropertyTypes = Enumerable.from(allPropertiesPerIntention)
                ...
                $scope.onSave = () => {
                    ...
                    if ($scope.input.Owner != null &&
                        $scope.input.Owner.DisplayName != null &&
                        $scope.input.Owner.Number != null) {
                        $scope.input.Owner.Contact = {
                            Phones: [
                                {
                                    Value: $scope.input.Owner.Number,
                ...
                $scope.onSaveAndConfirm = () => {
                    $scope.input.ConversionWarning = false;
                    $scope.input.IsHidden = false;
                    $scope.onSave();
                }
            }
        }
