module JahanJooy.ShishDong.Request {
    ...
    [
        ...
        "allEstateAreaRanges", "allUnitAreaRanges","allPropertiesPerUsage",
        ($scope,
            $timeout,
            ...
            $scope.$watch("searchInput.UsageType", u => {
                $scope.allPropertyTypes = Enumerable.from<any>($scope.allPropertyTypeOptions).where(p => Enumerable.from<any>(allPropertiesPerUsage).any(a => a.usage === u
            ...
            $scope.getDataDown = (index) => {
                if (!$scope.DataIsCompeleted) {
                    ...
                    $http.post("/api/haftdong/requests/search",
                        {
