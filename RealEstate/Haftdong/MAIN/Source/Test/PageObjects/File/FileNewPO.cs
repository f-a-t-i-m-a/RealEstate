...
{
    public class FileNewPO
    {
        private readonly IWebElement _elem;

        public FileNewPO(RemoteWebDriver drv)
        {
            _elem = drv.FindElement(By.CssSelector(".newFileView"));
        }
        ...
        public void SetNewOwner(string fullName, string phoneNumber)
        {
            ...
            RedioButtonPO radioButton = new RedioButtonPO(_elem, "customerType", "newCustomer");
            radioButton.Select();

            _elem.FindElement(By.CssSelector("input[ng-model*='customer.DisplayName']")).Clear();
            _elem.FindElement(By.CssSelector("input[ng-model*='customer.DisplayName']")).SendKeys(fullName);
            Thread.Sleep(2000);

            _elem.FindElement(By.CssSelector("input[ng-model*='customer.MobileNumber']")).Clear();
            _elem.FindElement(By.CssSelector("input[ng-model*='customer.MobileNumber']")).SendKeys(phoneNumber);
            Thread.Sleep(2000);
        }
