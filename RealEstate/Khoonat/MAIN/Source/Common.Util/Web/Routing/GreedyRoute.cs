...
namespace JahanJooy.Common.Util.Web.Routing
{
	...
	public class GreedyRoute : Route
	{
		...
		private readonly bool _hasGreedySegment;
		private readonly LinkedList<GreedyRouteSegment> _urlSegments = new LinkedList<GreedyRouteSegment>();
			...
			: base(url.Replace("*", ""), defaults, constraints, dataTokens, routeHandler)
		{
			...
			foreach (string segment in url.Split('/'))
			{
				...
				if (rx.IsMatch(segment))
				{
					...
					_urlSegments.AddLast(s);

					_hasGreedySegment |= s.IsGreedy;
			...
			LinkedListNode<GreedyRouteSegment> seg = _urlSegments.Last;
			int sIndex = _urlSegments.Count;
			...
			if (_hasGreedySegment)
			{
				LinkedListNode<GreedyRouteSegment> s = _urlSegments.Last;
		...
		private RouteValueDictionary ParseRoute(string virtualPath)
		{
			...
			LinkedListNode<GreedyRouteSegment> currentSegment = _urlSegments.First;
			...
			currentSegment = _urlSegments.Last;
			...
			if (!finished)
			{
				object remaining = string.Join("/", parts.Reverse().ToArray());
		...
		private RouteUrl Bind(RouteValueDictionary currentValues, RouteValueDictionary values)
		{
			...
			var required = new HashSet<string>(_urlSegments.Where(seg => seg.IsToken).ToList().ConvertAll(seg => seg.Name), StringComparer.OrdinalIgnoreCase);
			...
			foreach (GreedyRouteSegment seg in _urlSegments.Where(ss => ss.IsToken))
			{
			...
			bool mustAdd = _hasGreedySegment;
