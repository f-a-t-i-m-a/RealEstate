...
namespace JahanJooy.RealEstate.Core.Impl.Templates
{
	...
	public class RazorTemplateRunner
	{
		...
		public void Initialize()
		{
			...
			_host.DefaultBaseClass = typeof (TemplateBase).FullName;
			_host.DefaultNamespace = "RazorTemplateRunner";
			_host.DefaultClassName = "Template";
		...
		private Type BuildType(TextReader template)
		{
			...
			if (razorResult.ParserErrors != null && razorResult.ParserErrors.Count > 0)
				throw new ArgumentException("Error parsing razor template: " + razorResult.ParserErrors[0].Message + " @ " + razorResult.ParserErrors[0].Location);
				...
				new CompilerParameters(new[]
					                       {
											   ...
											   typeof(ServiceStack.TextExtensions).Assembly.CodeBase.Replace("file:///", "").Replace("/", "\\")
					                       }),
